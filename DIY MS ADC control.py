...
def main():                     # main program
    ...
    for runup in runups:
        ...
        with open(fn, 'x') as f:
            ...
            ADC.keypress(inputs[0])     # call for input#
            time.sleep(100E-3)
            ADC.keypress('B')           # call for mode B
            time.sleep(100E-3)
            ADC.keypress(runup)           # call for run-up mode
            ...
            def readntimes(n, key):
                for l2 in range (0, n):
                    if l2 == n-1: ADC.keypress(key)
                    actionresult = ADC.read()
                    #print('Key: ' + k)
                    #if l1 >= start:
                    if actionresult != 'Invalid':
                        values.append([ADC.k, 2*l2+1, actionresult[0]])
                        values.append([ADC.k, 2*l2+2, actionresult[1]])
                    else: print('Invalid')
    ...
    # get typical values
            ...
            ADC.ser.flushInput()              # discard serial buffer .read_all
            ...
            properscaleµV = (ADC.scale*1000)/(median7V - median0V) * ((median7V > 0) - (median7V < 0))  # (median7V > 0) - (median7V < 0) gives sign of median7V
            ...
            ADC.keypress(inputs[1])           # call for input
            time.sleep(100E-3)
            ADC.keypress('A')           # call for mode A (AZ mode)
            time.sleep(100E-3)
            ADC.keypress(runup)           # call for run-up mode
    ...
    # get typical values
        ...
        ADC.keypress('7')           # always end with 0V
